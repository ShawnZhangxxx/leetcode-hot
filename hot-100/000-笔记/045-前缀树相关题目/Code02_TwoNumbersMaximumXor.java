...
//把所有数的二进制放到前缀数中,之后循环一次就够了,每次找当前数最大的^值
...
public class Code02_TwoNumbersMaximumXor {
	...
	public static int maxXor(int num) {
		...
		for (int i = high, status, want; i >= 0; i--) {
			// status : num第i位的状态
			status = (num >> i) & 1;
			// want : num第i位希望遇到的状态 1 = status ^ want 不用判断status是0还是1了 秒啊!
			want = status ^ 1;
			if (tree[cur][want] == 0) { // 询问前缀树，能不能达成
				// 不能达成
				want ^= 1;   // status ^1 ^1 结果不变被 还是status
			}
			// want变成真的往下走的路
			ans |= (status ^ want) << i; //处理完最高那位
			cur = tree[cur][want]; //
		}
		return ans;
	}
	...
	public int findMaximumXOR2(int[] nums) {
		...
		for (int i = 31 - Integer.numberOfLeadingZeros(max); i >= 0; i--) {
			...
			int better = ans | (1 << i);
			set.clear();
			for (int num : nums) {//每次循环定ans最高位的是否有1
				// num : 31.....i 这些状态保留，剩下全成0
				num = (num >> i) << i;
				set.add(num);
				// num ^ 某状态 是否能 达成better目标，就在set中找 某状态 : better ^ num
				if (set.contains(better ^ num)) { //因为异或是互逆运算 better  =  num ^ element
					ans = better;
					break;
				}
			}
